...
---
---For compatibility reasons we still refer to `\pdf...` commands but *LuaTeX*
---has a different backend interface. Instead of these primitives there are three
---interfacing primitives: `pdfextension`, `pdfvariable` and `pdffeedback` that take keywords and optional further arguments (below we will
...
---as “just a number” and although it still has the same meaning (object
...
---    that is not a problem because of *Lua*'s `os.execute`), and the paranoia
---    checks on `openin` and `openout` do not happen. However, it is
...
---variant of `textdir` which injects the balanced node before the skip.
...
---can predict the expected behaviour. In fact, the `linedir` is just a
...
---needed.
---
---The `texmf.cnf` settings related to main memory are no longer used (these
---are: `main_memory`, `mem_bot`, `extra_mem_top` and `extra_mem_bot`). “Out of main memory” errors can still occur, but the
...
---relevant code now lives in the C file `texnode.c`, and basically uses a
---dozen or so “avail” lists instead of a doubly-linked model. An extra
...
---`vlink` and `vinfo` as well as `token_link` and `token_info`. All access to the variable memory array is now hidden behind a
---macro called `vmem`. We mention this because using the *TeX* book as
...
---limitation.
---
---A side-effect of the current implementation is that `global` is now more
