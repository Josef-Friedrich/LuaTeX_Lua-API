...
---\getbuffer
---
---As you can see, the `\global` prefix makes the value persistent outside the
...
---and normally that is more than enough. However, in addition in *Lua*METATEX\ we
...
---\typebuffer
---
---There are `integers`, `cardinals` and `floats` but the `numbers` one is the most generic as it tries to resolve it from one of these
...
---\typebuffer
---
---It typesets this (watch the `lua` variant, which is a precise
