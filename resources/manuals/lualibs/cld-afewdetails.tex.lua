...
---```
---
---Watch how the `modes` table lives in the `tex` namespace. We also
---have `systemmodes`. At the *TeX* end these are mode names preceded by a
---`*`, so the following code is similar:
---
---```
...
---register zero has type `string` and is treated as such.
---
---\startbuffer
...
---
---If we now say `\the\toks0` we will get {\tttf \the\toks0} as
...
---\typebuffer \getbuffer
---
---At the *TeX* end you can flush this box (`\box0`) or take a copy
---(`\copy0`). At the *Lua* end you would do:
---
---```
---context.copy()
---context.direct(0)
...
---        for n in node.traverse_id(nodes.nodecodes.glyph,head) do setcolor(n,color)
---        end
---        count = count + 1
---        return head, true
---    end
...
---\typebuffer
---
---The `\par` is needed because otherwise the processing is already disabled
...
---        if m then setcolor(n.list,"mymarker:" .. m)
---        end
---    end
---    return head, true
---end
