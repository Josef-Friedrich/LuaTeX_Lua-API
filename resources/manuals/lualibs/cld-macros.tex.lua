...
---```
---
---Here `#1` and `#2` represent an argument and there can be at most 9
---of them. The `[]` are delimiters and you can delimit in many ways so the
...
---\stopbuffer
---
---If you want to to define an environment (i.e.\ a `start`||`stop`
...
---\stoppacked
---
---The arguments are know in both `startmore` and `stopmore` and nesting
...
---return to *TeX*, so when we pipe the meaning of `TestB` back to *TeX* it
---only gets expanded afterwards. We can use a function to get back to *Lua*. It's
---only then that the meaning of `testC` is changed by the (piped) expansion
---of `TestB`.
---
---\startbuffer
---\startluacode
