...
---        \externalfigure [cld-mkiv-simple-titlepage.pdf] [height=.5\textheight]
---    } {
---        \scale [height=.5\textheight] {\cldprocessfile{cld-mkiv-simple-titlepage.cld}}
---    }
---\stopplacefigure
---
----------------------------------------------------------------
...
---    for i=1,10 do
---      context.bTR()
---      for i=1,20 do local r = random(99) context.bTD(r < 50 and one or two) context("%2i",r) context.eTD()
---      end
---      context.eTR()
---    end
---context.eTABLE()
...
---
---Here we see a function call to `context` in the most indented line. The
...
---we define the tables `one` and `two` beforehand. This saves 198
...
---defrost it. Functions in the `commands` namespace implement functionality
...
---*ConTeXt* distribution a more advanced version can be found in `s-edu-01.cld` as I was also asked to generate multiplication and table
...
---over each *UTF-8* character in a string. The other two helpers map a character onto
...
---Also, in order to support *Unicode* one could as well use the bits of *UTF-8*
...
---        local m = codes[s]
---        if m then if inmorse then MorseBetweenWords() else inmorse = true end local done = false for m in utfcharacters(m) do if done then MorseBetweenCharacters() else done = true end if m == "·" then MorseShort() elseif m == "—" then MorseLong() elseif m == " " then MorseBetweenCharacters() end end inmorse = true
---        elseif s == "\n" or s == " " then MorseSpace() inmorse = false
---        else if inmorse then MorseBetweenWords() else inmorse = true end MorseUnknown(s)
---        end
---    end
---end
---```
...
---splitter that splits on spacing tokens. The special case for `i = 1` makes
...
---end
---
---local splitter = lpeg.P(reset) * lpeg.splitter(lpeg.patterns.spacer,apply)
---
---local function process(data)
...
---
---Sometimes can save a bit of work by using formatters. By default, the `context`
...
---\stopbuffer
---
---The last one is the most interesting one here: in the subnamespace `formatted` (watch the `d`) a format specification with extra arguments is
---expected.
---
---\ctxluabuffer
---
