...
---`lucida-math` that is used as part of the font specification.
---
---```
...
---the following case the file `dingbats.lfg` gets loaded (only once) when the
...
---\stopsmalltyping
---
---Here the `px-math` virtual font is defined. A series of fonts is loaded and
---combined into one. The `vector` entry is used to tell the builder how to
...
---
---In addition to the official `ssty` feature for enforcing usage of script
...
---\stopsmalltyping
---
---These can be activated (in math mode) with the `\mathalternate` command
---like:
---
---```
---`\mathalternate{cal}Z`
...
---Latin Modern has design sizes. The values (like `twelve` are tables defined
...
---\stopsmalltyping
---
---The `tounicode` option makes sure that additional information ends ip in
