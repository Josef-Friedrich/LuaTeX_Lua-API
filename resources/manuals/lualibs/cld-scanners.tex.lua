...
---Of course in practice the `actions` will be more complex.
---
...
---        public    = true,
---        arguments = "string",
---        actions   = function(s) context("(%s)",s)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---
---By default a macro gets defined in the `\clf_` namespace but the `public` option makes it visible. This default indicates that it is actually a
...
---
---We can also define the macro to be protected (`\unexpanded`) in *ConTeXt*
---speak). We can overload existing scanners but unless we specify the `overload` option, we get a warning on the console.
---
---\startbuffer[definition]
---\startluacode
---    interfaces.implement {
...
---        actions   = function(s,i,b,d) context("<%s> <%i> <%l> <%p>",s,i,b,d)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---\getbuffer[usage]
---
---As you can see, registers can be used as well, and the `\temp` macro is
...
---        public    = true,
---        arguments = "number",
---        actions   = function(f) context("<%.2f>",f)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        public    = true,
---        arguments = "list",
---        actions   = function(t) context("< % + t >",t)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        name      = "MyMacroE",
---        public    = true,
---        arguments = { { { "bar", "integer" }, { "foo", "dimen" }, }
---        },
---        actions   = function(t) context("<foo : %p> <bar : %i>",t.foo,t.bar)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        name      = "MyMacroF",
---        public    = true,
---        arguments = { "string", { { "bar", "integer" }, { "foo", "dimen" }, }, { { "one", "string" }, { "two", "string" }, },
---        },
---        actions   = function(s,t1,t2) context("<%s> <%p> <%i> <%s> <%s>",s,t1.foo,t1.bar,t2.one,t2.two)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        name      = "MyMacroG",
---        public    = true,
---        arguments = { "string", "string", { { "data", "string" }, { "tab", "string" }, { "method", "string" }, { "foo", { { "method", "integer" }, { "compact", "number" }, { "nature" }, { "*" }, -- any key } }, { "compact", "string", "tonumber" }, { "nature", "boolean" }, { "escape" }, }, "boolean",
---        },
---        actions   = function(s1, s2, t, b) context("<%s> <%s>",s1,s2) context("<%s> <%s> <%s>",t.data,t.tab,t.compact) context("<%i> <%s> <%s>",t.foo.method,t.foo.nature,t.foo.whatever) context("<%l>",b)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---\stopbuffer
---
---Although the `\relax` is not really needed in the next calls, I often use
...
---
---Expansion effects are most noticeable when we pickup a “string” kind of
---value. In the implementor we have two methods for that: `string` and `argument`. The argument method has an expandable form (the default) and one
...
---        name      = "MyMacroH",
---        public    = true,
---        arguments = { "string", "argument", "argumentasis",
---        },
---        actions   = function(a,b,c) context.type(a or "-") context.quad() context.type(b or "-") context.quad() context.type(c or "-") context.crlf()
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---\typebuffer[usage]
---
---We we use the string method we need a `\relax` (or some spacer) to end
...
---        name      = "MyMacroI",
---        public    = true,
---        arguments = { "argument", "argumentasis",
---        },
---        actions   = function(a,b,c) context.type(a or "-") context.quad() context.type(b or "-") context.quad() context.type(c or "-") context.crlf()
---        end,
---    }
...
---        arguments = { "hbox", "vbox",
---        },
---        actions   = function(h,v) context(h) context(v)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        name      = "MyMacroN",
---        public    = true,
---        arguments = { "hash", "array",
---        },
---        actions   = function(h, a) context.totable(h) context.quad() context.totable(a)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        public    = true,
---        arguments = "verbatim",
---        actions   = function(v) context.type(v)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        public    = true,
---        arguments = "csname",
---        actions   = function(c) context("{\\ttbf name:} {\\tttf %s}",c)
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---        public    = true,
---        arguments = "toks",
---        actions   = function(t) context("%S : ",t) context.sprint(t) context.crlf()
---        end,
---    }
---\stopluacode
---\stopbuffer
...
---
---The plural `actions` suggests that there can be more than one and indeed
...
---        if sign then context("`%i + %i = %i`",a,b,a+b)
---        else context("`%i - %i = %i`",a,b,a-b)
---        end
---    end
---
---    interfaces.implement {
...
---\getbuffer[usage]
---
---If you need to pass a string, you pass it as `"'preset'"`, so single quotes
...
---you have not defined `\foo` as expandable macro. When it is unexpandable it
---will get passed as it is and *Lua* will see a `\f` as an escaped character.
---So, when you want a macro be passes as macro, you need to do it as in the third
---line. The fact that there is a comment trigger (`--`) doesn't help here.
---
---```
---\startluacode
---    context("foo")
...
---```
---
---When you use `\ctxlua` the same is true but there you also need to keep an
---eye on special characters. For instance a percent sign is then interpreted in the
---*TeX* way and because all becomes one line, a `--` somewhere in the middle
...
---that follows. You can use `\letterpercent` to smuggle a percent sign in a
---`\ctxlua` call. Special characters like a hash symbol also need to be
...
---external file and load that one (e.g. with `require`.
---
...
---        name      = "TestFloat",
---        public    = true,
---        actions   = function(b) if b then context("%q",F) else F = scan_number() end
---        end,
---        valuetype = "none",
---    }
---
...
---        name      = "TestThis",
---        public    = true,
---        actions   = function(b) if b then return random(scan_integer(),scan_integer()) else randomseed(scan_integer(true)) end
---        end,
---        valuetype = "count",
---    }
---
...
---        name    = "whateverbftwo",
---        public  = true,
---        actions = function() ctx_whateverbfxxx(false)
---        end
---    }
---
---    interfaces.implement {
