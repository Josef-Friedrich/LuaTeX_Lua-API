...
---\stoplinecorrection
---
---I the first loop we can use the format options associated with the simple `context` call. This will not work in the second case. Even worse, passing more
...
---\typebuffer
---
---Watch the call to `color`, this will pass definitions at the *TeX* end to
...
---```
---
---to get a graphic that has its own page. Don't use the `metafun` namespace
...
---```
---
---We use `gm` (Graphic Magic) for the conversion and pass the old and new
...
---with suffix `bmp` can be included you have to register it as follows. The
