...
local err = function(...)
    return luatexbase.module_error("luamplib",
                                   select("#", ...) > 1 and format(...) or ...)
end
local warn = function(...)
    return luatexbase.module_warning("luamplib", select("#", ...) > 1 and
                                         format(...) or ...)
end
local info = function(...)
    return luatexbase.module_info("luamplib",
                                  select("#", ...) > 1 and format(...) or ...)
end
...
local stripsuffix = file.stripsuffix or
                        function(filename)
...
if lfstouch then
    ...
    if texmfvar and texmfvar ~= "" and texmfvar ~= '$TEXMFVAR' then
        for _, dir in next,
                      texmfvar:explode(os.type == "windows" and ";" or ":") do
            if not lfsisdir(dir) then mk_full_path(dir) end
...
function luamplib.getcachedir(dir)
    dir = dir:gsub("##", "#")
    dir = dir:gsub("^~", os.type == "windows" and os.getenv("UserProfile") or
                       os.getenv("HOME"))
...
local function replaceformatmp(file, newfile, ofmodify)
    ...
    fh = ioopen(newfile, "w")
    if not fh then return file end
    fh:write("let normalinfont = infont;\n",
...
local name_e = "%f[^%a_]"
local btex_etex =
    name_b .. "btex" .. name_e .. "%s*(.-)%s*" .. name_b .. "etex" .. name_e
local verbatimtex_etex = name_b .. "verbatimtex" .. name_e .. "%s*(.-)%s*" ..
                             name_b .. "etex" .. name_e
...
local function replaceinputmpfile(name, file)
    ...
    if newfile and luamplibtime then
        ...
        if nf and nf.mode == "file" and ofmodify == nf.modification and
            luamplibtime < nf.access then
            return nf.size == 0 and file or newfile
        end
    end

    if name == "format.mp" then
        return replaceformatmp(file, newfile, ofmodify)
    end
...
do
    local exe = 0
    while arg[exe - 1] do exe = exe - 1 end
    mpkpse = kpse.new(arg[exe], "mpost")
end

local special_ftype = {pfb = "type1 fonts", enc = "enc files"}
...
local function reporterror(result, indeed)
    if not result then
        ...
        if result.status > 0 then
            warn(log)
            if result.status > 1 then err(e or "see above messages") end
        elseif indeed then
...
local function setformat(name) currentformat = name end
...
local function process_indeed(mpx, data)
    ...
    if mpx and data then
        ...
        err(
            "Mem file unloadable. Maybe generated with a different version of mplib?")
    end
...
local function process(data, instancename)
    local defaultinstancename = currentformat ..
...
local function process_color(str)
    if str then
        if not str:find("{.-}") then str = format("{%s}", str) end
        run_tex_code(mplibcolor_fmt:format(str), catat11)
        return format('1 withprescript "MPlibOverrideColor=%s"',
                      texgettoks "mplibtmptoks")
    end
    return ""
end
...
local function process_dimen(str)
    if str then
        str = str:gsub("{(.+)}", "%1")
        run_tex_code(format(
                         [[\mplibtmptoks\expandafter{\the\dimexpr %s\relax}]],
                         str))
...
local function process_verbatimtex_text(str)
    if str then run_tex_code(str) end
    return ""
end
...
local function process_verbatimtex_prefig(str)
    if str then tex_code_pre_mplib[luamplib.figid] = str end
    return ""
end

local function process_verbatimtex_infig(str)
    if str then return format('special "postmplibverbtex=%s";', str) end
    return ""
end
...
function luamplib.runscript(code)
    ...
    if type(f) == "function" then
        local buffer = {}
        function mp.print(...) mpprint(buffer, ...) end
        f()
        ---@diagnostic disable-next-line: cast-local-type
        buffer = tableconcat(buffer)
        if buffer and buffer ~= "" then return buffer end
        buffer = {}
        mpprint(buffer, f())
...
local function protecttexcontents(str)
    return str:gsub("\\%%", "\0PerCent\0"):gsub("%%.-\n", ""):gsub("%%.-$", "")
               :gsub("%zPerCent%z", "\\%%"):gsub("%s+", " ")
end
...
local function protect_expansion(str)
    if str then
        str = str:gsub("\\", "!!!Control!!!"):gsub("%%", "!!!Comment!!!"):gsub(
                  "#", "!!!HashSign!!!"):gsub("{", "!!!LBrace!!!"):gsub("}",
                                                                        "!!!RBrace!!!")
...
local function unprotect_expansion(str)
    if str then
        return str:gsub("!!!Control!!!", "\\"):gsub("!!!Comment!!!", "%%"):gsub(
                   "!!!HashSign!!!", "#"):gsub("!!!LBrace!!!", "{"):gsub(
                   "!!!RBrace!!!", "}")
    end
end
...
local function getobjects(result, figure, f) return figure:objects() end
...
local function pdf_stopfigure() texsprint("\\mplibstoptoPDF") end
...
local function pdf_textfigure(font, size, text, width, height, depth)
    ...
    texsprint(format("\\mplibtextext{%s}{%f}{%s}{%s}{%f}", font, size, text, 0,
                     -(7200 / 7227) / 65536 * depth))
end
...
local function pen_characteristics(object)
    ...
    divider = sx * sy - rx * ry
    return
        not (sx == 1 and rx == 0 and ry == 0 and sy == 1 and tx == 0 and ty == 0),
        t.width
end
...
local function flushnormalpath(path, open)
    ...
    for i = 1, #path do
        ...
        elseif curved(ith, pth) then
            pdf_literalcode("%f %f %f %f %f %f c", ith.right_x, ith.right_y,
                            pth.left_x, pth.left_y, pth.x_coord, pth.y_coord)
        else
    ...
    if not open then
        ...
        if curved(pth, one) then
            pdf_literalcode("%f %f %f %f %f %f c", pth.right_x, pth.right_y,
                            one.left_x, one.left_y, one.x_coord, one.y_coord)
        else
...
local function flushconcatpath(path, open)
    ...
    for i = 1, #path do
        ...
        elseif curved(ith, pth) then
            ...
            pdf_literalcode("%f %f %f %f %f %f c", a, b, c, d,
                            concat(pth.x_coord, pth.y_coord))
        else
    ...
    if not open then
        ...
        if curved(pth, one) then
            ...
            pdf_literalcode("%f %f %f %f %f %f c", a, b, c, d,
                            concat(one.x_coord, one.y_coord))
        else
...
local pdfoutput = tonumber(texget("outputmode")) or
                      tonumber(texget("pdfoutput"))
...
local pgf = {
    bye = "pgfutil@everybye",
    extgs = "pgf@sys@addpdfresource@extgs@plain"
}
...
local function update_pdfobjs(os)
    ...
    if on then return on, false end
    if pdfmode then
...
local function update_tr_res(res, mode, opaq)
    ...
    if new then
        if pdfmode then
            ...
            if pgf.loaded then
                texsprint(format("\\csname %s\\endcsname{/MPlibTr%i%s}",
                                 pgf.extgs, on, os))
            else
                texsprint(format("\\special{pdf:put @MPlibTr<</MPlibTr%i%s>>}",
                                 on, os))
            end
        end
    end
...
local function tr_pdf_pageresources(mode, opaq)
    ...
    if pdfmode then
        ...
        if not pgf.loaded then
            texsprint(format(
                          "\\special{pdf:put @resources<</ExtGState @MPlibTr>>}"))
        end
    end
    return on_on, off_on
end
...
local function shading_initialize()
    ...
    if pdfmode and luatexbase.callbacktypes.finish_pdffile then -- ltluatex
        ...
        luatexbase.add_to_callback("finish_pdffile", function()
            pdf.immediateobj(shading_obj,
                             format("<<%s>>", tableconcat(shading_res)))
        end, "luamplib.finish_pdffile")
        pdf_objs.finishpdf = true
    end
end

local function sh_pdfpageresources(shtype, domain, colorspace, colora, colorb,
                                   coordinates)
    if not shading_res then shading_initialize() end
    local os = format(
                   "<</FunctionType 2/Domain [ %s ]/C0 [ %s ]/C1 [ %s ]/N 1>>",
                   domain, colora, colorb)
    local funcobj = pdfmode and format("%i 0 R", update_pdfobjs(os)) or os
    os = format(
             "<</ShadingType %i/ColorSpace /%s/Function %s/Coords [ %s ]/Extend [ true true ]/AntiAlias true>>",
    ...
    if pdfmode then
        ...
        if new then
            texsprint(format("\\special{pdf:put @MPlibSh<</MPlibSh%i%s>>}", on,
                             os))
        end
...
local function do_preobj_color(object, prescript)
    ...
    if sh_type then
        ...
        for _, t in pairs({centera, centerb}) do
            for i, v in ipairs(t) do t[i] = format("%f", v) end
        end
        ...
        for _, t in pairs({colora, colorb}) do
            for i, v in ipairs(t) do t[i] = format("%.3f", v) end
        end
...
local function do_postobj_color(tr, over, sh)
    if sh then pdf_literalcode("W n /MPlibSh%s sh Q", sh) end
    if over then texsprint("\\special{color pop}") end
    if tr then pdf_literalcode("/MPlibTr%i gs", tr) end
end
    ...
    if result then
        ...
        if figures then
            ...
            for f = 1, #figures do
                ...
                local fignum = tonumber(figure:filename():match("([%d]+)$") or
                ...
                if urx < llx then
                    ...
                    if objects then
                        ...
                        for o = 1, #objects do
                            ...
                            local tr_opaq, cr_over, shade_no = do_preobj_color(
                                                                   object,
                                                                   prescript)
                            if prescript and prescript.mplibtexboxid then
                                put_tex_boxes(object, prescript)
                            elseif objecttype == "start_bounds" or objecttype ==
                                "stop_bounds" then -- skip
                            elseif objecttype == "start_clip" then
                                local evenodd =
                                    not object.istext and object.postscript ==
                                        "evenodd"
                                start_pdf_code()
                            ...
                            elseif objecttype == "stop_clip" then
                                stop_pdf_code()
                                miterlimit, linecap, linejoin, dashed = -1, -1,
                                                                        -1,
                                                                        false
                            elseif objecttype == "special" then
                                if prescript and prescript.postmplibverbtex then
                                    TeX_code_bot[#TeX_code_bot + 1] =
                                        prescript.postmplibverbtex
                                end
                            elseif objecttype == "text" then
                                local ot = object.transform -- 3,4,5,6,1,2
                                start_pdf_code()
                                pdf_literalcode("%f %f %f %f %f %f cm", ot[3],
                                                ot[4], ot[5], ot[6], ot[1],
                                                ot[2])
                                pdf_textfigure(object.font, object.dsize,
                                               object.text, object.width,
                                               object.height, object.depth)
                                stop_pdf_code()
                            else
                                local evenodd, collect, both = false, false,
                                                               false
                                ...
                                if collect then
                                    if not savedpath then
                                        ...
                                        savedpath[#savedpath + 1] =
                                            object.path or false
                                        savedhtap[#savedhtap + 1] =
                                            object.htap or false
                                    end
                                else
                                    ...
                                    local open =
                                        path and path[1].left_type and
                                            path[#path].right_type
                                    local pen = object.pen
                                    if pen then
                                        if pen.type == 'elliptical' then
                                            transformed, penwidth =
                                                pen_characteristics(object) -- boolean, value
                                    ...
                                    if path then
                                        ...
                                        if objecttype == "fill" then
                                            pdf_literalcode(
                                                evenodd and "h f*" or "h f")
                                        elseif objecttype == "outline" then
                                            pdf_literalcode(open and "S" or
                                                                "h S")
                                        elseif objecttype == "both" then
                                            pdf_literalcode(
                                                evenodd and "h B*" or "h B")
                                        end
                                        if transformed then
                                            stop_pdf_code()
                                        end
...
local function colorconverter(cr)
    ...
    if n == 4 then
        ...
        return
            format("%.3f %.3f %.3f %.3f k %.3f %.3f %.3f %.3f K", c, m, y, k, c,
                   m, y, k), "0 g 0 G"
    elseif n == 3 then
