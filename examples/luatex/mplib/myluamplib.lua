...
local err = function(...)
  return luatexbase.module_error(
    "luamplib",
    select("#", ...) > 1 and format(...) or ...
  )
end
local warn = function(...)
  return luatexbase.module_warning(
    "luamplib",
    select("#", ...) > 1 and format(...) or ...
  )
end
local info = function(...)
  return luatexbase.module_info(
    "luamplib",
    select("#", ...) > 1 and format(...) or ...
  )
end
...
local replacesuffix = file.replacesuffix
  or function(filename, suffix)
    return (filename:gsub("%.[%a%d]+$", "")) .. "." .. suffix
  end
local stripsuffix = file.stripsuffix
...
local function replaceinputmpfile(name, file)
  ...
  if newfile and luamplibtime then
    ...
    if
      nf
      and nf.mode == "file"
      and ofmodify == nf.modification
      and luamplibtime < nf.access
    then
...
if tonumber(mplib.version()) <= 1.50 then
  err(
    "luamplib no longer supports mplib v1.50 or lower. "
...
local function reporterror(result, indeed)
  if not result then
    ...
    log =
      log:gsub("%(Please type a command or say `end'%)", ""):gsub("\n+", "\n")
...
local function process_color(str)
  if str then
    ...
    return format(
      '1 withprescript "MPlibOverrideColor=%s"',
...
local function process_dimen(str)
  if str then
    str = str:gsub("{(.+)}", "%1")
    run_tex_code(
      format([[\mplibtmptoks\expandafter{\the\dimexpr %s\relax}]], str)
    )
...
function luamplib.maketext(str, what)
  if str and str ~= "" then
    ...
    if what == 1 then
      if
        not str:find("\\documentclass" .. name_e)
        and not str:find("\\begin%s*{document}")
        and not str:find("\\documentstyle" .. name_e)
        and not str:find("\\usepackage" .. name_e)
      then
        if luamplib.legacy_verbatimtex then
...
local function curved(
  ...
  if
    abs(ith.right_x - ith.x_coord - d) <= bend_tolerance
    and abs(pth.x_coord - pth.left_x - d) <= bend_tolerance
  then
    d = pth.left_y - ith.right_y
    if
      abs(ith.right_y - ith.y_coord - d) <= bend_tolerance
      and abs(pth.y_coord - pth.left_y - d) <= bend_tolerance
    then
      return false
    end
  end
...
local pdfoutput = tonumber(texget("outputmode"))
  or tonumber(texget("pdfoutput"))
...
if pdfmode then -- repect luaotfload-colors
  getpageres = pdf.getpageresources
    or function()
      return pdf.pageresources
    end
  setpageres = pdf.setpageresources
    or function(s)
      pdf.pageresources = s
    end
else
...
local function update_tr_res(res, mode, opaq)
  ...
  if new then
    if pdfmode then
      ...
      if pgf.loaded then
        texsprint(
          format("\\csname %s\\endcsname{/MPlibTr%i%s}", pgf.extgs, on, os)
        )
      else
...
local function sh_pdfpageresources(
  shtype,
  domain,
  colorspace,
  colora,
...
local function do_preobj_color(object, prescript)
  ...
  if sh_type then
    ...
    if sh_type == "linear" then
      ...
      shade_no =
        sh_pdfpageresources(2, domain, colorspace, colora, colorb, coordinates)
    elseif sh_type == "circular" then
      local radiusa = format("%f", prescript.sh_radius_a)
      local radiusb = format("%f", prescript.sh_radius_b)
      local coordinates =
        tableconcat({ centera, radiusa, centerb, radiusb }, " ")
      shade_no =
        sh_pdfpageresources(3, domain, colorspace, colora, colorb, coordinates)
    end
  ...
  if result then
    ...
    if figures then
      for f = 1, #figures do
        ...
        local fignum = tonumber(
          figure:filename():match("([%d]+)$") or figure:charcode() or 0
        )
        ...
        if urx < llx then
          ...
          if objects then
            ...
            for o = 1, #objects do
              ...
              local tr_opaq, cr_over, shade_no =
                do_preobj_color(object, prescript)
              if prescript and prescript.mplibtexboxid then
                put_tex_boxes(object, prescript)
              elseif
                objecttype == "start_bounds" or objecttype == "stop_bounds"
              then -- skip
              elseif objecttype == "start_clip" then
                local evenodd = not object.istext
                  and object.postscript == "evenodd"
                start_pdf_code()
              ...
              elseif objecttype == "text" then
                ...
                if collect then
                  ...
                  local open = path
                    and path[1].left_type
                    and path[#path].right_type
