...
\href{https://github.com/Josef-Friedrich/LuaTeX_Lua-API/blob/main/library/luametatex/xmath.lua}{library/luametatex/lmathx.lua}
...
\section{Navigation table \texttt{\_N}}

% Einige Lua-Dateien enthalten eine Tabelle mit dem Namen |_N|.
Some Lua files contain a table named |_N|.
% |_N| steht für \emph{Navigation}.
|_N| stands for \emph{navigation}.
% Mithilfe dieser Tabelle und der Gliederungsansicht des Editors ist die Navigation durch die Dokumentation einfacher.
With the help of this table and the outline view of the editor, it is
easier to navigate through the documentation.
% Der Name ist von der globalen Lua-Tabelle |_G| inspiriert.
The name is inspired by the global Lua table |_G|.
% Viele Teile der Dokumentation, wie beispielsweise die Definition der verschiedenen Knotenklassen, werden in der Gliederung nicht angezeigt.
Many parts of the documentation, such as the
definition of the various `Node` classes, are not shown in the outline.
% In der veröffentlichten Version wird diese Navigationstabelle mithilfe des Verwaltungsskripts manage.py entfernt.
...
% Die verschiedenen Node-Typen werden als Klassen definiert.
The different node types are defined as classes.
% Da diese Klassendefinition komplett in den Kommentaren statt findet, wird sie nicht in der Outline angezeigt.
Since this class definition takes place entirely in the comments, it is not displayed in the outline.
...
% Das folgende Beispiel verweist auf das Kapitel „8.7.2 is_node“ auf Seite 149 in der LuaTeX Dokumentation.
The following example refers to section “8.7.2 is\_node” on page 149 in the Lua\TeX{} documentation.
